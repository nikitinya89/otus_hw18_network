...
sysctl -p
```
Чтобы организовать доступ в Интернет со всех серверов через **inetRouter*, необходимо настроить NAT. Для указания всех подсетей, будем использовать более широкую маску /16:
```bash
...
```bash
nasible-playbook network.yaml
```
Ansible выполняет следующие действия:
- удаляет автоматически маршрут умолчанию, изменив конфигурацию netplan
- с помощью netplan добавляет недостающие маршруты
- разрешает IP Forwarding
- настраивает NAT на inetRouter и добавляет скрипт по восстановление правил iptables в автозапуск
- перезагружает все сервера  

Проверяем, что после перезагрузки все работает.
